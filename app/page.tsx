...
type StockItem = { id: string; name: string; code: string; units: string; health: number; color: "blue" | "amber" | "rose" };
...
export default function Home() {
  ...
  const [stockModal, setStockModal] = useState<null | { mode: "create" | "edit"; stock?: StockItem }>(null);
  const [stocks, setStocks] = useState<StockItem[]>([
    { id: "FG", name: "Finished Goods", code: "FG-STORE", units: "1,248", health: 96, color: "blue" },
    { id: "STEEL", name: "Steel Raw Material", code: "RM-STEEL", units: "846", health: 82, color: "amber" },
    { id: "PACK", name: "Packaging Components", code: "PK-STORE", units: "386", health: 74, color: "rose" },
  ]);
  const [stockDraft, setStockDraft] = useState({ name: "", code: "", units: "0" });
  ...
  return (
    <main className="min-h-screen bg-[#eef1f4] text-[#20252d]">
      <div className="flex min-h-screen">
        <aside className={`fixed inset-y-0 left-0 z-40 flex flex-col bg-[#272c33] text-slate-300 transition-all duration-300 lg:static ${collapsed ? "w-[76px]" : "w-[256px]"} ${mobileMenu ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
          <div className="flex h-[82px] items-center border-b border-white/10 px-5">
            <button onClick={() => setCollapsed(!collapsed)} className="mr-3 rounded-lg p-2 text-slate-400 transition hover:bg-white/10 hover:text-white" aria-label={collapsed ? "Expand sidebar" : "Collapse sidebar"}><Menu className="size-5" /></button>
        ...
        <section className="min-w-0 flex-1 nexus-enter-soft">
          ...
          <div className="px-5 py-6 sm:px-8 lg:px-10 lg:py-8">
            <div className="mx-auto max-w-[1400px]">
              ...
              <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{stats.map(({ label, value, detail, icon: Icon, tone }, index) => <article key={label} style={{ animationDelay: `${index * 80}ms` }} className="nexus-card nexus-enter nexus-3d rounded-xl border border-slate-200/80 bg-white p-5 shadow-[0_2px_8px_rgba(30,40,50,0.03)]"><div className="flex items-start justify-between"><div className={`flex size-10 items-center justify-center rounded-lg ${tone === "blue" ? "bg-blue-50 text-blue-600" : tone === "amber" ? "bg-amber-50 text-amber-600" : tone === "rose" ? "bg-rose-50 text-rose-600" : "bg-emerald-50 text-emerald-600"}`}><Icon className="size-[18px]" /></div><button className="text-slate-300 hover:text-slate-500" aria-label={`More options for ${label}`}><MoreHorizontal className="size-5" /></button></div><p className="mt-5 text-xs font-medium text-slate-500">{label}</p><p className="mt-1 text-2xl font-bold tracking-tight text-slate-800">{value}</p><p className="mt-2 text-[11px] text-slate-400">{detail}</p></article>)}</div>

              <section className="nexus-card nexus-glow nexus-enter mt-6 overflow-hidden rounded-2xl border border-slate-200/80 bg-white" style={{ animationDelay: "320ms" }}>
                <div className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-100 px-5 py-5 sm:px-6"><div className="flex items-center gap-3"><div className="flex size-10 items-center justify-center rounded-xl bg-[#272c33] text-[#d8a64e]"><Boxes className="size-5" /></div><div><div className="flex items-center gap-2"><h3 className="text-sm font-bold text-slate-800">Stock command center</h3><span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[9px] font-bold uppercase tracking-wider text-emerald-700"><span className="nexus-live size-1.5 rounded-full bg-emerald-500" />Realtime</span></div><p className="mt-1 text-xs text-slate-400">Manage independent stock spaces for FG, steel, packaging and more.</p></div></div><button onClick={() => { setStockDraft({ name: "", code: "", units: "0" }); setStockModal({ mode: "create" }); }} className="flex items-center gap-2 rounded-lg bg-[#272c33] px-3.5 py-2.5 text-xs font-bold text-white shadow-sm transition hover:-translate-y-0.5 hover:bg-slate-700"><Plus className="size-4" />Create stock</button></div>
                <div className="grid gap-3 p-5 sm:grid-cols-2 xl:grid-cols-3 sm:p-6">{stocks.map((stock) => <article key={stock.id} className="nexus-3d group relative overflow-hidden rounded-xl border border-slate-100 bg-slate-50/70 p-4 transition hover:-translate-y-1 hover:border-[#d8a64e]/50 hover:bg-white"><div className="flex items-start justify-between"><div className={`flex size-9 items-center justify-center rounded-lg ${stock.color === "blue" ? "bg-blue-100 text-blue-600" : stock.color === "amber" ? "bg-amber-100 text-amber-600" : "bg-rose-100 text-rose-600"}`}><Box className="size-4" /></div><div className="flex gap-1 opacity-0 transition group-hover:opacity-100"><button onClick={() => { setStockDraft({ name: stock.name, code: stock.code, units: stock.units }); setStockModal({ mode: "edit", stock }); }} className="rounded-md p-1.5 text-slate-400 hover:bg-white hover:text-slate-700" aria-label={`Edit ${stock.name}`}><Pencil className="size-3.5" /></button><button onClick={() => setStocks(stocks.filter((item) => item.id !== stock.id))} className="rounded-md p-1.5 text-slate-400 hover:bg-rose-50 hover:text-rose-600" aria-label={`Delete ${stock.name}`}><Trash2 className="size-3.5" /></button></div></div><p className="mt-4 text-xs font-bold text-slate-800">{stock.name}</p><p className="mt-1 text-[10px] font-medium tracking-wider text-slate-400">{stock.code}</p><div className="mt-4 flex items-end justify-between"><div><p className="text-[10px] text-slate-400">Available units</p><p className="mt-1 text-xl font-bold tracking-tight text-slate-800">{stock.units}</p></div><div className="text-right"><p className="text-[10px] text-slate-400">Health</p><p className="mt-1 text-sm font-bold text-emerald-600">{stock.health}%</p></div></div><div className="mt-3 h-1 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-gradient-to-r from-[#6d8fd9] to-[#d8a64e] transition-all duration-700" style={{ width: `${stock.health}%` }} /></div></article>)}</div>
              </section>
      ...
      {stockModal && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm"><div className="nexus-modal w-full max-w-md rounded-2xl border border-slate-200 bg-white p-6 shadow-2xl"><div className="flex items-start justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#a2782f]">Stock workspace</p><h2 className="mt-1 text-lg font-bold text-slate-800">{stockModal.mode === "create" ? "Create a new stock" : "Edit stock details"}</h2><p className="mt-1 text-xs text-slate-400">Define a dedicated space for your operational inventory.</p></div><button onClick={() => setStockModal(null)} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100" aria-label="Close stock dialog"><X className="size-4" /></button></div><div className="mt-6 flex flex-col gap-4"><label className="flex flex-col gap-2 text-xs font-semibold text-slate-600">Stock name<input value={stockDraft.name} onChange={(event) => setStockDraft({ ...stockDraft, name: event.target.value })} placeholder="e.g. Aluminum raw material" className="h-11 rounded-lg border border-slate-200 px-3 text-sm font-normal text-slate-800 outline-none transition focus:border-[#d8a64e] focus:ring-2 focus:ring-[#d8a64e]/20" /></label><label className="flex flex-col gap-2 text-xs font-semibold text-slate-600">Stock code<input value={stockDraft.code} onChange={(event) => setStockDraft({ ...stockDraft, code: event.target.value.toUpperCase() })} placeholder="e.g. RM-ALU" className="h-11 rounded-lg border border-slate-200 px-3 text-sm font-normal text-slate-800 outline-none transition focus:border-[#d8a64e] focus:ring-2 focus:ring-[#d8a64e]/20" /></label><label className="flex flex-col gap-2 text-xs font-semibold text-slate-600">Opening units<input value={stockDraft.units} onChange={(event) => setStockDraft({ ...stockDraft, units: event.target.value })} type="number" min="0" className="h-11 rounded-lg border border-slate-200 px-3 text-sm font-normal text-slate-800 outline-none transition focus:border-[#d8a64e] focus:ring-2 focus:ring-[#d8a64e]/20" /></label></div><div className="mt-7 flex justify-end gap-3"><button onClick={() => setStockModal(null)} className="rounded-lg px-4 py-2.5 text-xs font-semibold text-slate-500 hover:bg-slate-100">Cancel</button><button disabled={!stockDraft.name.trim() || !stockDraft.code.trim()} onClick={() => { if (stockModal.mode === "create") setStocks([...stocks, { id: `${stockDraft.code}-${Date.now()}`, name: stockDraft.name, code: stockDraft.code, units: Number(stockDraft.units || 0).toLocaleString(), health: 88, color: "blue" }]); else if (stockModal.stock) setStocks(stocks.map((item) => item.id === stockModal.stock?.id ? { ...item, name: stockDraft.name, code: stockDraft.code, units: Number(stockDraft.units || 0).toLocaleString() } : item)); setStockModal(null); }} className="rounded-lg bg-[#272c33] px-4 py-2.5 text-xs font-bold text-white transition hover:bg-slate-700 disabled:cursor-not-allowed disabled:opacity-40">{stockModal.mode === "create" ? "Create stock" : "Save changes"}</button></div></div></div>}
